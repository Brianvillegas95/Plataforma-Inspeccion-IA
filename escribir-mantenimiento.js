...
// --- (checkForOpenDuplicate, findMechanicByName, findMechanicActiveJob, getMechanicQueueCounts, findMechanicToAssign, updateMechanicStatus - SIN CAMBIOS) ---
...
// (updateMechanicAvailability - SIN CAMBIOS)
async function updateMechanicAvailability(sheets, name, availability) {
  ...
  let newStatusSistema = 'Ocupado'; // Default
  ...
  // Si es 'No Disponible', el 'newStatusSistema' no se actualizará aquí,
  // la lógica de logout lo manejará por separado.
...
// --- ¡FUNCIÓN MODIFICADA! ---
// Ahora solo busca trabajos "En Espera"
async function findNextJobInSharedQueue(sheets, mechanicArea) {
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    // ¡CAMBIO CLAVE! Solo busca trabajos "En Espera" en la cola compartida
    if (normJobArea === normMechanicArea && mecanicoAsignado === 'En Espera' && jobStatusParo === 'En Cola') {
      ...
      if (jobStatusMaquina === 'trabajando' && !lowPriorityJob) {
        lowPriorityJob = { row: row }; // Guardamos el primero 'trabajando'
      }
    }
  }
...
// (reAssignPendingJob - SIN CAMBIOS)
async function reAssignPendingJob(sheets, mechanic, jobRow) {
  console.log(`RE-ASIGNACIÓN dinámica (al cerrar/check-in): ${mechanic.name} -> Fila ${jobRow}`);
...
// --- ¡FUNCIÓN MODIFICADA! ---
// Renombrada para mayor claridad. Ahora "jala" 1 trabajo.
async function findAndAssignNextJob(sheets, mechanicName) {
  ...
  // 1. Busca el próximo trabajo "En Espera"
  ...
  if (nextJob) {
      ...
      // 2b. No hay cola. Se asegura que esté "Libre"
...
exports.handler = async function (event) {
  ...
  try {
    ...
    switch (action) {
      ...
      // --- Acción 'abrir' (Sin cambios en su lógica) ---
      case 'abrir': {
        ...
        const mechanicToAssign = await findMechanicToAssign(sheets, areaDelParo); // Solo busca 'Libres'
        let statusParo;
      ...
      case 'cerrar': {
        ...
        // 1. Marcar el trabajo como "Cerrado"
        await sheets.spreadsheets.values.batchUpdate({
          spreadsheetId: produccionSheetId,
          resource: {
            valueInputOption: 'USER_ENTERED',
        ...
        // 2. ¡CAMBIO! Liberar al mecánico Y que "jale" el próximo trabajo "En Espera"
        if (mecanicoAsignadoOriginal && mecanicoAsignadoOriginal !== 'En Espera') {
            ...
            // Esta función lo pondrá "Libre" y le asignará un nuevo trabajo si existe
      ...
      // --- Acción 'mecanico_check_in' (MODIFICADA) ---
      ...
      // --- Acción 'escalar_paro' (Sin cambios) ---
      case 'escalar_paro': {
      ...
      // --- Acción 'check_status' (Sin cambios) ---
      case 'check_status': {
        if (!row) return { statusCode: 400, body: JSON.stringify({ error: 'Falta "row".' }) };
        const response = await sheets.spreadsheets.values.get({
          spreadsheetId: produccionSheetId,
      ...
      // --- Acción 'mecanico_check_out' (MODIFICADA) ---
      case 'mecanico_check_out': {
        if (!name) return { statusCode: 400, body: JSON.stringify({ error: 'Falta "name".' }) };
        
        const mechanic = await findMechanicByName(sheets, name);
        if (mechanic) {
            // 1. Liberar todos sus trabajos y ponerlos "En Espera"
      ...
      // --- Acción 'get_mecanico_tareas' (MODIFICADA) ---
      ...
      // (get_mecanicos_activos - SIN CAMBIOS)
